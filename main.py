...
category_list = {'films': 'Фильмы', 'serials': 'Сериалы', 'series': 'Сериалы', 'cartoons': 'Мультфильмы', 'cartoon': 'Мультфильмы', 'cartoon-serials': 'Мультсериалы', 'cartoon-series': 'Мультсериалы',
                 'anime-film': 'Аниме-фильмы', 'anime': 'Аниме-фильмы', 'anime-serials': 'Аниме-сериалы', 'anime-series': 'Аниме-сериалы', 'tv-shows': 'ТВ-Шоу', 'tv-show': 'ТВ-Шоу', 'film': 'Фильмы'}
last_domain = ''
...
async def update_popular():
    ...
    for result in results:
        ...
        try:
            ...
            genre = ''
        popular_films.append({'id': film_id, 'name': name, 'year': year, 'quality': quality, 'genre': genre,
                             'type': type, 'country': country, 'poster': poster, 'kinopoisk': kinopoisk, 'imdb': imdb})
    ...
    for result in results:
        ...
        try:
            ...
            genre = ''
        popular_series.append({'id': film_id, 'name': name, 'year': year, 'quality': quality, 'genre': genre,
                              'type': type, 'country': country, 'poster': poster, 'kinopoisk': kinopoisk, 'imdb': imdb})
...
async def update_popular_mult():
    ...
    for result in results:
        ...
        try:
            ...
            genre = ''
        popular_cartoon.append({'id': film_id, 'name': name, 'year': year, 'quality': quality, 'genre': genre,
                               'type': type, 'country': country, 'poster': poster, 'kinopoisk': kinopoisk, 'imdb': imdb})
    ...
    for result in results:
        ...
        try:
            ...
            genre = ''
        popular_cartoon_serials.append({'id': film_id, 'name': name, 'year': year, 'quality': quality, 'genre': genre,
                                       'type': type, 'country': country, 'poster': poster, 'kinopoisk': kinopoisk, 'imdb': imdb})
...
async def update_popular_anime():
    ...
    for result in results:
        ...
        try:
            ...
            genre = ''
        popular_anime.append({'id': film_id, 'name': name, 'year': year, 'quality': quality, 'genre': genre,
                             'type': type, 'country': country, 'poster': poster, 'kinopoisk': kinopoisk, 'imdb': imdb})
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(f'https://api1673051707.bhcesh.me/list?token=3794a7638b5863cc60d7b2b9274fa32e&sort=-views&type=anime-serials&join_seasons=false&limit=50&year=2024', timeout=None) as response:
            response = await response.json()
    ...
    for result in results:
        ...
        try:
            ...
            genre = ''
        popular_anime_serials.append({'id': film_id, 'name': name, 'year': year, 'quality': quality, 'genre': genre,
                                     'type': type, 'country': country, 'poster': poster, 'imdb': imdb, 'kinopoisk': kinopoisk})
...
async def update_popular_show():
    ...
    for result in results:
        ...
        try:
            ...
            genre = ''
        popular_show.append({'id': film_id, 'name': name, 'year': year, 'quality': quality, 'genre': genre,
                            'type': type, 'country': country, 'poster': poster, 'imdb': imdb, 'kinopoisk': kinopoisk})
...
async def update_news_films():
    ...
    for result in results:
        ...
        try:
            ...
            genre = ''
        news_films.append({'id': film_id, 'name': name, 'year': year, 'quality': quality, 'genre': genre,
                          'type': type, 'country': country, 'poster': poster, 'imdb': imdb, 'kinopoisk': kinopoisk})
...
async def update_news_serials():
    ...
    for result in results:
        ...
        try:
            ...
            genre = ''
        news_serials.append({'id': film_id, 'name': name, 'year': year, 'quality': quality, 'genre': genre,
                            'type': type, 'country': country, 'poster': poster, 'imdb': imdb, 'kinopoisk': kinopoisk})
        await asyncio.sleep(3)
...
async def update_news_show():
    ...
    for result in results:
        ...
        try:
            ...
            genre = ''
        news_show.append({'id': film_id, 'name': name, 'year': year, 'quality': quality, 'genre': genre,
                         'type': type, 'country': country, 'poster': poster, 'imdb': imdb, 'kinopoisk': kinopoisk})
        await asyncio.sleep(3)
...
async def update_collections():
    ...
    with open('collections.json', 'r', encoding="utf-8") as f:
        ...
        collections_data = [(collection_items[0], collection_items[1])
                            for collection_items in collections_data]
    async with aiohttp.ClientSession() as session:
    ...
    for page in range(1, pages+1):
        async with aiohttp.ClientSession() as session:
            async with session.get(f'https://api1662739038.bhcesh.me/collection?page={page}&token=3794a7638b5863cc60d7b2b9274fa32e') as response:
                response = await response.json()
        collection_results = [(collection_items['id'], collection_items['name'])
                              for collection_items in response['results']]
        collections_data = list(set(collections_data + collection_results))
        await asyncio.sleep(3)
    collections['data'] = sorted(
        collections_data, key=lambda k: k[0], reverse=False)
    current_date = date.today()
            ...
            for film in films:
                ...
                if str(film['id']) not in collection_ids:
                    ...
                    try:
                        ...
                        genre = ''
                    data = [collection_id, film['id'], film['name'], genre,
                            film["year"], film['iframe_url'], film['poster'], film["type"]]
                    print(data)
                    db.add_film(data)
...
async def send_welcome(message: types.Message, state: FSMContext):
    ...
    if user_status.status == 'member' or user_status.status == 'administrator' or user_status.status == 'creator':
        text = f'<a href="https://gifbot.netlify.app/templates/hello.jpg">🎞️</a> <b><u>Kinozzz Bot</u></b> — уникальный в своём роде бот, который даст Вам возможность <b>бесплатно</b> наслаждаться новинками <b>Российского</b> и <b>зарубежного</b> кинематографа с <u>любого устройства</u>.'
        await bot.send_message(message.from_user.id, f'{text}', reply_markup=inlinekeyboard)
    else:
        not_subscribed_text = "🤖 Для максимального использования функционала бота, подпишитесь на наш канал! 💬🔔"
        subscribe_button = InlineKeyboardButton(
            "Подписаться", url=f"https://t.me/{channel_id[1:]}")
        subscribe_keyboard = InlineKeyboardMarkup().add(subscribe_button)
        await bot.send_message(message.from_user.id, not_subscribed_text, reply_markup=subscribe_keyboard)
...
async def popular_menu(call: types.CallbackQuery):
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='<a href="https://gifbot.netlify.app/templates/popular.jpg">🚀</a> Вы перешли в раздел <b>«Популярное»</b>, здесь находятся видеоматериалы, которые <b>популярны</b> на текущий год.\n\n<i>👉 Выберите <b>категорию</b>, в которой желаете выбрать <b>видеоматериал</b> для просмотра.</i>', reply_markup=popular_menu_kb)


@dp.callback_query_handler(text="news_menu", state="*")
async def news_menu(call: types.CallbackQuery):
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='<a href="https://gifbot.netlify.app/templates/news.jpg">🆕</a> Вы перешли в раздел <b>«Новинки»</b>, здесь находятся <b>новые</b> видеоматериалы, которые были <b>добавлены</b> в течении последних суток.\n\n<i>👉 Выберите <b>категорию</b>, в которой желаете выбрать <b>видеоматериал</b> для просмотра.</i>', reply_markup=news_menu_kb)
...
async def news_menu(call: types.CallbackQuery):
    ...
    pagination = NewsPagination(films=popular_films, width=2,
                                back_prefix="news_films_back_", next_prefix="news_films_next_")
    kb = pagination.get_page_keyboard(
        cur_page=1, fave_status=film_id in favorite_ids)
...
async def next(call: types.CallbackQuery):
    ...
    pagination = NewsPagination(films=popular_films, width=2,
                                back_prefix="news_films_back_", next_prefix="news_films_next_")
    kb = pagination.get_page_keyboard(
        cur_page=call.data, fave_status=film_id in favorite_ids)
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=f'<b><a href="{poster}">▶️</a> Название:</b> {name}\n<b>🏅 КП:</b> {kinopoisk} | <b>IMDb:</b> {imdb}\n<b>🌍 Страна:</b> {country}\n<b>📀 Качество:</b> {quality}\n<b>📁 Категория:</b> {type}\n<b>🎦 Жанр:</b> {genre}\n<b>🗓️ Год:</b> {year}', reply_markup=kb)
...
async def news_menu(call: types.CallbackQuery):
    ...
    pagination = NewsPagination(films=popular_films, width=2,
                                back_prefix="news_serials_back_", next_prefix="news_serials_next_")
    kb = pagination.get_page_keyboard(
        cur_page=1, fave_status=film_id in favorite_ids)
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=f'<b><a href="{poster}">▶️</a> Название:</b> {name}\n<b>🏅 КП:</b> {kinopoisk} | <b>IMDb:</b> {imdb}\n<b>🌍 Страна:</b> {country}\n<b>📀 Качество:</b> {quality}\n<b>📁 Категория:</b> {type}\n<b>🎦 Жанр:</b> {genre}\n<b>🗓️ Год:</b> {year}', reply_markup=kb)
...
async def news_menu(call: types.CallbackQuery):
    ...
    pagination = NewsPagination(films=popular_films, width=2,
                                back_prefix="news_show_back_", next_prefix="news_show_next_")
    kb = pagination.get_page_keyboard(
        cur_page=1, fave_status=film_id in favorite_ids)
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=f'<b><a href="{poster}">▶️</a> Название:</b> {name}\n<b>🏅 КП:</b> {kinopoisk} | <b>IMDb:</b> {imdb}\n<b>🌍 Страна:</b> {country}\n<b>📀 Качество:</b> {quality}\n<b>📁 Категория:</b> {type}\n<b>🎦 Жанр:</b> {genre}\n<b>🗓️ Год:</b> {year}', reply_markup=kb)
...
async def popular_menu(call: types.CallbackQuery):
    ...
    pagination = NewsPagination(films=popular_films, width=2,
                                back_prefix="popular_filmsback_", next_prefix="popular_filmsnext_")
    kb = pagination.get_page_keyboard(
        cur_page=1, fave_status=film_id in favorite_ids)
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=f'<b><a href="{poster}">▶️</a> Название:</b> {name}\n<b>🏅 КП:</b> {kinopoisk} | <b>IMDb:</b> {imdb}\n<b>🌍 Страна:</b> {country}\n<b>📀 Качество:</b> {quality}\n<b>📁 Категория:</b> {type}\n<b>🎦 Жанр:</b> {genre}\n<b>🗓️ Год:</b> {year}', reply_markup=kb)
...
async def popular_menu(call: types.CallbackQuery):
    ...
    pagination = NewsPagination(films=popular_films, width=2,
                                back_prefix="popular_seriesback_", next_prefix="popular_seriesnext_")
    kb = pagination.get_page_keyboard(
        cur_page=1, fave_status=film_id in favorite_ids)
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=f'<b><a href="{poster}">▶️</a> Название:</b> {name}\n<b>🏅 КП:</b> {kinopoisk} | <b>IMDb:</b> {imdb}\n<b>🌍 Страна:</b> {country}\n<b>📀 Качество:</b> {quality}\n<b>📁 Категория:</b> {type}\n<b>🎦 Жанр:</b> {genre}\n<b>🗓️ Год:</b> {year}', reply_markup=kb)
...
async def popular_menu(call: types.CallbackQuery):
    ...
    pagination = NewsPagination(films=popular_films, width=2,
                                back_prefix="popular_cartoonback_", next_prefix="popular_cartoonnext_")
    kb = pagination.get_page_keyboard(
        cur_page=1, fave_status=film_id in favorite_ids)
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=f'<b><a href="{poster}">▶️</a> Название:</b> {name}\n<b>🏅 КП:</b> {kinopoisk} | <b>IMDb:</b> {imdb}\n<b>🌍 Страна:</b> {country}\n<b>📀 Качество:</b> {quality}\n<b>📁 Категория:</b> {type}\n<b>🎦 Жанр:</b> {genre}\n<b>🗓️ Год:</b> {year}', reply_markup=kb)
...
async def popular_menu(call: types.CallbackQuery):
    ...
    pagination = NewsPagination(films=popular_films, width=2,
                                back_prefix="popular_cartoon_serials_back_", next_prefix="popular_cartoon_serials_next_")
    kb = pagination.get_page_keyboard(
        cur_page=1, fave_status=film_id in favorite_ids)
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=f'<b><a href="{poster}">▶️</a> Название:</b> {name}\n<b>🏅 КП:</b> {kinopoisk} | <b>IMDb:</b> {imdb}\n<b>🌍 Страна:</b> {country}\n<b>📀 Качество:</b> {quality}\n<b>📁 Категория:</b> {type}\n<b>🎦 Жанр:</b> {genre}\n<b>🗓️ Год:</b> {year}', reply_markup=kb)
...
async def popular_menu(call: types.CallbackQuery):
    ...
    pagination = NewsPagination(films=popular_films, width=2,
                                back_prefix="popular_animeback_", next_prefix="popular_animenext_")
    kb = pagination.get_page_keyboard(
        cur_page=1, fave_status=film_id in favorite_ids)
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=f'<b><a href="{poster}">▶️</a> Название:</b> {name}\n<b>🏅 КП:</b> {kinopoisk} | <b>IMDb:</b> {imdb}\n<b>🌍 Страна:</b> {country}\n<b>📀 Качество:</b> {quality}\n<b>📁 Категория:</b> {type}\n<b>🎦 Жанр:</b> {genre}\n<b>🗓️ Год:</b> {year}', reply_markup=kb)
...
async def popular_menu(call: types.CallbackQuery):
    ...
    pagination = NewsPagination(films=popular_films, width=2,
                                back_prefix="popular_anime_serials_back_", next_prefix="popular_anime_serials_next_")
    kb = pagination.get_page_keyboard(
        cur_page=1, fave_status=film_id in favorite_ids)
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=f'<b><a href="{poster}">▶️</a> Название:</b> {name}\n<b>🏅 КП:</b> {kinopoisk} | <b>IMDb:</b> {imdb}\n<b>🌍 Страна:</b> {country}\n<b>📀 Качество:</b> {quality}\n<b>📁 Категория:</b> {type}\n<b>🎦 Жанр:</b> {genre}\n<b>🗓️ Год:</b> {year}', reply_markup=kb)
...
async def popular_menu(call: types.CallbackQuery):
    ...
    pagination = NewsPagination(films=popular_films, width=2,
                                back_prefix="popular_showback_", next_prefix="popular_shownext_")
    kb = pagination.get_page_keyboard(
        cur_page=1, fave_status=film_id in favorite_ids)
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=f'<b><a href="{poster}">▶️</a> Название:</b> {name}\n<b>🏅 КП:</b> {kinopoisk} | <b>IMDb:</b> {imdb}\n<b>🌍 Страна:</b> {country}\n<b>📀 Качество:</b> {quality}\n<b>📁 Категория:</b> {type}\n<b>🎦 Жанр:</b> {genre}\n<b>🗓️ Год:</b> {year}', reply_markup=kb)
...
async def send(call: types.CallbackQuery):
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='<a href="https://gifbot.netlify.app/templates/searcg.jpg">🔍</a> Вы перешли в раздел <b>«Поиск»</b>, пожалуйста, выберите <b>тип поиска</b>, которым желаете воспользоваться.', reply_markup=category)


@dp.callback_query_handler(text="categories", state="*")
async def send(call: types.CallbackQuery):
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='<a href="https://gifbot.netlify.app/templates/searcg.jpg">🔍</a> Вы перешли в раздел <b>«Поиск по названию»</b>, пожалуйста, выберите <b>категорию</b>, в которой желаете найти <b>видеоматериал</b>.', reply_markup=category)
...
async def send(call: types.CallbackQuery):
    with open('collections.json', 'r', encoding="utf-8") as f:
        collections = json.load(f)
    pagination = InlinePagination(button_datas=[(
        collection_items[1], collection_items[0]) for collection_items in collections['data']], width=2)
    kb = pagination.get_page_keyboard(cur_page=1)
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='<a href="https://gifbot.netlify.app/templates/podborki.jpg">🎞️</a> Вы перешли в раздел <b>«Подборки»</b>, пожалуйста, выберите <b>подборку</b>, в которой желаете выбрать <b>видеоматериал</b> для просмотра.', reply_markup=kb)
    ...
    if film_id not in favorites_ids:
        ...
        if len(film_data) == 0:
            ...
            film_data = requests.get(
                "https://api1663355922.bhcesh.me/franchise/details?token=3794a7638b5863cc60d7b2b9274fa32e", params=params).json()
            try:
                genre = film_data['genre'].values()
                genre = ', '.join(genre)
            except KeyError:
                genre = ''
            data = [film_id, call.message.chat.id, film_data['name'], film_data['year'],
                    genre, film_data["iframe_url"], film_data['poster'], film_data['type']]
        else:
            film_data = film_data[0]
            data = [film_id, call.message.chat.id, film_data[2], film_data[4],
                    film_data[3], film_data[5], film_data[6], film_data[7]]
...
async def next(call: types.CallbackQuery):
    ...
    kb = pagination.get_page_keyboard(
        cur_page=1, collection_id=collection_id, fave_status=film_id in favorite_ids)
...
async def next(call: types.CallbackQuery):
    ...
    type = category_list[type]
    kb = pagination.get_page_keyboard(
        cur_page=call.data, collection_id=collection_id, fave_status=film_id in favorite_ids)
    kb.row(InlineKeyboardButton(text="🏠 Вернуться в меню", callback_data="back"))
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=f'<b><a href="{poster}">▶️</a> Название:</b> {name}\n<b>📁 Категория:</b> {type}\n<b>🎦 Жанр:</b> {genre}\n<b>🗓️ Год:</b> {year}', reply_markup=kb)
...
async def send(call: types.CallbackQuery):
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='<a href="https://gifbot.netlify.app/templates/about.jpg">🎞️</a> <b><u>Kinozzz Bot</u></b> — <b>первый онлайн-кинотеатр</b> в Telegram, который предоставляет возможность <b><u>бесплатно</u></b> наслаждаться новинками <b>Российского</b> и <b>зарубежного</b> кинематографа.\n\n💡 <b>Основные возможности бота:</b>\n— Удобный поиск фильмов, сериалов, ТВ-шоу, мультфильмов и т.п. по названию;\n— <b>Подборки</b> видеоматериалов;\n— Функция <b>«Мои закладки»</b>, чтобы любимые фильмы и сериалы были всегда рядом;\n— Удобный плеер;\n— Высокое качество каждого видеоматериала;\n— Ежедневное пополнение новинками кино.', reply_markup=about)


@dp.callback_query_handler(text="contacts", state="*")
async def send(call: types.CallbackQuery):
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='<b><a href="https://gifbot.netlify.app/templates/svyaz.jpg">🔥</a> Мы всегда рады новым идеям и предложениям, которые будут полезны для нашей площадки!</b>\n\n📌 <i>В случае возникновения жалоб на авторские права, обращайтесь на <b>@yusufbekovv24</b></i>', reply_markup=contacts)
...
async def statistic(message: types.Message):
    if message.from_user.id == admin_id:
        ...
        inlinekeyboard_stats.add(types.InlineKeyboardButton(
            text="Общая статистика", callback_data="stat_all"))
        inlinekeyboard_stats.add(types.InlineKeyboardButton(
            text="Статистика за сутки", callback_data="stat_day"))
...
async def send(call: types.CallbackQuery):
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='<a href="https://gifbot.netlify.app/templates/searchfilm.jpg">🔎</a> Укажите оригинальное название <b>фильма</b> для поиска.\n\n❗<i><b>Важно:</b> указывайте точное название для корректного поиска.</i>', reply_markup=inlinekeyboard2)
    ...
    async def handle_text(message: types.Message, state: FSMContext):
        ...
        response = requests.get(
            "https://api1650820663.bhcesh.me/list?token=3794a7638b5863cc60d7b2b9274fa32e", params=params).json()['results']
        try:
            if str(response).count('activate_time') > 1:
                ...
                async def handle_text(message: types.Message, state: FSMContext):
                    ...
                    try:
                        country = response[int(
                            message.text)]["country"].values()
                        country = ', '.join(country)
                    except:
                        country = ''
                    ...
                    play.add(types.InlineKeyboardButton(
                        text="😍 Смотреть онлайн", url=f'https://kino79.netlify.app'))
                    if film_id in favorite_ids:
                        play.row(InlineKeyboardButton(
                            text="❌ Удалить из закладок", callback_data=f'del_favorite|{film_id}'))
                    else:
                        play.row(InlineKeyboardButton(
                            text="➕ Добавить в закладки", callback_data=f'add_favorite|{film_id}'))
            ...
            await message.answer('<a href="https://gifbot.netlify.app/templates/nosearch.jpg">😔</a> Не удалось найти <b>фильм</b> с подобным названием.\n\n❗<i>Введите корректное название для более точного поиска.</i>', reply_markup=inlinekeyboard2)
            await state.finish()
...
async def start(callback_query: types.CallbackQuery, state: FSMContext):
    await state.finish()
    await bot.send_message(callback_query.from_user.id, "🎥 <b>Введите ID фильма, чтобы смотреть.</b>\n\n ID можно найти на наших социальных сетях.📱\n", reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("Ссылка на TikTok", url="https://www.tiktok.com/@cinema_magicbot?_t=8kFxhBw8e4m&_r=1")))
    await GetUserInfo.us_zapros_film_id.set()

@dp.message_handler(state=GetUserInfo.us_zapros_film_id)
async def id_text(message: types.Message, state: FSMContext):
    await state.update_data(us_zapros_film_id=message.text)
    data = await state.get_data()
    params = { "id": data['us_zapros_film_id']}
    id = data['us_zapros_film_id']
    response = requests.get(f"https://api1663355922.bhcesh.me/franchise/details?token=3794a7638b5863cc60d7b2b9274fa32e&id={id}", params=params).json()
    if response.get('id'):
        film_id = response['id']
        name = response['name']
        poster = response['poster']
        year = response['year']
        genre = ', '.join(response['genre'].values())
        country = ', '.join(response["country"].values())
        kinopoisk = response.get("kinopoisk")
        imdb = response.get("imdb")
        ...
        play.add(InlineKeyboardButton(
            text="😍 Смотреть онлайн", url=f'https://bot.kinozzz.monster/play/?id={film_id}'))
        if film_id in favorite_ids:
            play.row(InlineKeyboardButton(
                text="❌ Удалить из закладок", callback_data=f'del_favorite|{film_id}'))
        else:
        ...
        await message.answer('<a href="https://gifbot.netlify.app/templates/nosearch.jpg">😔</a> Не удалось найти <b>фильм</b> с подобным ID.\n\n❗<i>Введите корректный ID для более точного поиска.</i>', reply_markup=inlinekeyboard2)
        await state.finish()
...
async def search_films(query_text: str):
    ...
    response = requests.get(
        "https://api1650820663.bhcesh.me/list?token=3794a7638b5863cc60d7b2b9274fa32e", params=params).json()
...
async def inline_search(query: types.InlineQuery):
    ...
    results = await search_films(query_text)

    inline_results = []
    for result in results:
        title = f"{result['name']} | ({result['year']})"
        description = category_list.get(
            result['type'], '') + ' \n ' + ', '.join(result['genre'].values())
        thumb_url = result['poster']
        message_text = category_list.get(
            result['type'], '') + ' \n ' + ', '.join(result['genre'].values())
        ...
        inline_results.append(
            types.InlineQueryResultArticle(
                ...
                input_message_content=types.InputTextMessageContent(
                    message_text=f'<b><a href="{result["poster"]}">▶️</a> Название:</b> {result["name"]}\n<b>🏅 КП:</b> {result["kinopoisk"]} | <b>IMDb:</b> {result["imdb"]}\n<b>🌍 Страна:</b>{", ".join(result["country"].values())}\n<b>📀 Качество:</b> {result["quality"]}\n<b>📁 Категория: </b> {category_list[result["type"]]}\n<b>🎦 Жанр:</b> {", ".join(result["genre"].values())}\n<b>🗓️ Год:</b> {result["year"]}'
                ),
                reply_markup=types.InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            types.InlineKeyboardButton(
                                text="😍Смотреть", url=f'https://bot.kinozzz.monster/play/?id={film_id}'
                            ),
                            types.InlineKeyboardButton(
                                text="🤖Бот", url='https://t.me/Cinema_Magicbot'
                            ),
                        ]
                    ]
                ),
...
@dp.callback_query_handler(lambda query: query.data == "quick_search")
async def quick_search(callback_query: types.CallbackQuery):
    await bot.send_message(callback_query.from_user.id, "@Cinema_Magicbot")



@dp.callback_query_handler(text="serials", state="*")
async def send(call: types.CallbackQuery):
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='<a href="https://gifbot.netlify.app/templates/search-serial.jpg">🔎</a> Укажите оригинальное название <b>сериала</b> для поиска.\n\n❗<i><b>Важно:</b> указывайте точное название для корректного поиска.</i>', reply_markup=inlinekeyboard3)
    ...
    async def handle_text(message: types.Message, state: FSMContext):
        ...
        try:
            if str(response).count('activate_time') > 1:
                ...
                async def handle_text(message: types.Message, state: FSMContext):
                    data = await state.get_data()
                    params = {"type": 'serials', "name": {
                        data['us_zapros_serial']}}
                    response = requests.get(
                        "https://api1650820663.bhcesh.me/list?token=3794a7638b5863cc60d7b2b9274fa32e", params=params).json()['results']
                    film_id = response[int(message.text)]['id']
                    name = response[int(message.text)]['name']
                    ...
                    play = types.InlineKeyboardMarkup()
                    play.add(types.InlineKeyboardButton(
                        text="😍 Смотреть онлайн", url=f'https://kino79.netlify.app/{film_id}'))
                    if film_id in favorite_ids:
                        play.row(InlineKeyboardButton(
                            text="❌ Удалить из закладок", callback_data=f'del_favorite|{film_id}'))
                    else:
            ...
            await message.answer('<a href="https://gifbot.netlify.app/templates/nosearch.jpg">😔</a> Не удалось найти <b>сериал</b> с подобным названием.\n\n❗<i>Введите корректное название для более точного поиска.</i>', reply_markup=inlinekeyboard3)
            await state.finish()


@dp.callback_query_handler(text="anime_films", state="*")
async def send(call: types.CallbackQuery):
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='<a href="https://gifbot.netlify.app/templates/anime.jpg">🔎</a> Укажите оригинальное название <b>аниме-фильма</b> для поиска.\n\n❗<i><b>Важно:</b> указывайте точное название для корректного поиска.</i>', reply_markup=inlinekeyboard4)
    ...
    async def handle_text(message: types.Message, state: FSMContext):
        ...
        try:
            if str(response).count('activate_time') > 1:
                ...
                async def handle_text(message: types.Message, state: FSMContext):
                    data = await state.get_data()
                    params = {"type": 'anime', "name": {
                        data['us_zapros_animefilm']}}
                    response = requests.get(
                        "https://api1650820663.bhcesh.me/list?token=3794a7638b5863cc60d7b2b9274fa32e", params=params).json()['results']
                    film_id = response[int(message.text)]['id']
                    name = response[int(message.text)]['name']
            ...
            await message.answer('<a href="https://gifbot.netlify.app/templates/nosearch.jpg">😔</a> Не удалось найти <b>аниме-фильм</b> с подобным названием.\n\n❗<i>Введите корректное название для более точного поиска.</i>', reply_markup=inlinekeyboard4)
            await state.finish()


@dp.callback_query_handler(text="cartoon_serials", state="*")
async def send(call: types.CallbackQuery):
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='<a href="https://gifbot.netlify.app/templates/cartoon-serial.jpg">🔎</a> Укажите оригинальное название <b>мульт-сериала</b> для поиска.\n\n❗<i><b>Важно:</b> указывайте точное название для корректного поиска.</i>', reply_markup=inlinekeyboard5)
    ...
    async def handle_text(message: types.Message, state: FSMContext):
        ...
        data = await state.get_data()
        params = {"type": 'cartoon-serials',
                  "name": {data['us_zapros_cartoonser']}}
        response = requests.get(
            "https://api1650820663.bhcesh.me/list?token=3794a7638b5863cc60d7b2b9274fa32e", params=params).json()['results']
        try:
            if str(response).count('activate_time') > 1:
            ...
            print(ex)
            await message.answer('<a href="https://gifbot.netlify.app/templates/nosearch.jpg">😔</a> Не удалось найти <b>мультсериал</b> с подобным названием.\n\n❗<i>Введите корректное название для более точного поиска.</i>', reply_markup=inlinekeyboard5)
            await state.finish()


@dp.callback_query_handler(text="cartoon", state="*")
async def send(call: types.CallbackQuery):
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='<a href="https://gifbot.netlify.app/templates/cartoon.jpg">🔎</a> Укажите оригинальное название <b>мультфильма</b> для поиска.\n\n❗<i><b>Важно:</b> указывайте точное название для корректного поиска.</i>', reply_markup=inlinekeyboard6)
    ...
    async def handle_text(message: types.Message, state: FSMContext):
        ...
        try:
            if str(response).count('activate_time') > 1:
                ...
                async def handle_text(message: types.Message, state: FSMContext):
                    data = await state.get_data()
                    params = {"type": 'cartoon', "name": {
                        data['us_zapros_cartoon']}}
                    response = requests.get(
                        "https://api1650820663.bhcesh.me/list?token=3794a7638b5863cc60d7b2b9274fa32e", params=params).json()['results']
                    film_id = response[int(message.text)]['id']
                    name = response[int(message.text)]['name']
            ...
            await message.answer('<a href="https://gifbot.netlify.app/templates/nosearch.jpg">😔</a> Не удалось найти <b>мультфильм</b> с подобным названием.\n\n❗<i>Введите корректное название для более точного поиска.</i>', reply_markup=inlinekeyboard6)
            await state.finish()


@dp.callback_query_handler(text="anime_serials", state="*")
async def send(call: types.CallbackQuery):
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='<a href="https://gifbot.netlify.app/templates/anime.jpg">🔎</a> Укажите оригинальное название <b>аниме-сериала</b> для поиска.\n\n❗<i><b>Важно:</b> указывайте точное название для корректного поиска.</i>', reply_markup=inlinekeyboard7)
    ...
    async def handle_text(message: types.Message, state: FSMContext):
        ...
        data = await state.get_data()
        params = {"type": 'anime-serials',
                  "name": {data['us_zapros_animeser']}}
        response = requests.get(
            "https://api1650820663.bhcesh.me/list?token=3794a7638b5863cc60d7b2b9274fa32e", params=params).json()['results']
        try:
            if str(response).count('activate_time') > 1:
            ...
            await message.answer('<a href="https://gifbot.netlify.app/templates/nosearch.jpg">😔</a> Не удалось найти <b>аниме-сериал</b> с подобным названием.\n\n❗<i>Введите корректное название для более точного поиска.</i>', reply_markup=inlinekeyboard7)
            await state.finish()


@dp.callback_query_handler(text="tv", state="*")
async def send(call: types.CallbackQuery):
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='<a href="https://gifbot.netlify.app/templates/tv-show.jpg">🔎</a> Укажите оригинальное название <b>ТВ-шоу</b> для поиска.\n\n❗<i><b>Важно:</b> указывайте точное название для корректного поиска.</i>', reply_markup=inlinekeyboard8)
    ...
    async def handle_text(message: types.Message, state: FSMContext):
        ...
        try:
            ...
            await message.answer('<a href="https://gifbot.netlify.app/templates/nosearch.jpg">😔</a> Не удалось найти <b>ТВ-Шоу</b> с подобным названием.\n\n❗<i>Введите корректное название для более точного поиска.</i>', reply_markup=inlinekeyboard8)
            await state.finish()


@dp.message_handler(content_types=['text'])
async def send_all(message):
    await bot.send_message(message.from_user.id, f'❗К сожалению, ваш запрос не распознан!\n\n<a href="">🏠</a> <i>Вернитесь в <b>главное меню</b></i>.', reply_markup=exit)


@dp.callback_query_handler(text="back", state="*")
async def back(call: types.CallbackQuery, state: FSMContext):
    await state.finish()
    await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='🏠 Вы вернулись в <b>главное меню</b>.\n\n<a href="https://gifbot.netlify.app/templates/hello.jpg">🎦</a> Здесь вы можете выбрать <b>раздел</b>, в котором желаете найти или выбрать видеоматериал для просмотра.', reply_markup=inlinekeyboard, inline_message_id=call.inline_message_id)
